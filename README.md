...
스캔 간격은 **적응형**입니다. 동시 High인 Cursor가 충분하면 15분마다, 모든 Cursor 프로세스 중 Normal(목표 미달)이 하나라도 있거나 High가 부족하면 15초마다 확인합니다.
...
| 주기 | 적응형 — High &lt; 6 또는 Cursor 중 Normal(목표 미달) 1개 이상 → **15초**, High ≥ 6 · 전부 High → **15분** |
| 우선순위 | **High (높음)** |
...
flowchart TD
    ...
    F -->|예| H{아무 Cursor나 Normal? 또는 High개수 < 6?}
    G --> H
    H -->|예| I[다음 간격 15초]
...
3. **동시 High인 Cursor ≥ 6** 이고 전부 High이면 스캔 간격을 **15분**으로 늘립니다.
4. **어떤 Cursor 프로세스든** 다시 **Normal**(목표 미달)이면 간격을 **15초**로 되돌립니다.
...
| `$FastScanIntervalSeconds` | `15` | 빠른 스캔 주기(초). High &lt; 임계값, 또는 Cursor 중 하나라도 목표 미달일 때 |
| `$SlowScanIntervalSeconds` | `900` | 느린 스캔 주기(초, 기본 15분). High ≥ 임계값이고 전부 목표일 때 |
